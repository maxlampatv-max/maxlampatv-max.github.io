(function() {
  ...
  var Defined = {
    use_api: 'xsena',
    localhost: 'http://api.xsena.red/sisi',
    framework: ''
  };

  var luid = Lampa.Storage.get('lampac_unic_id', 'mq0vudvo');
  if (!luid) {
...
var hostkey = 'http://api.xsena.red'.replace('http://', '').replace('https://', '');
...
window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
  if (!window.rch_nws[hostkey].startTypeInvoke) {
    ...
    if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
      ...
      net.silent('http://api.xsena.red'.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check', function() {
        check(true);
      }, function() {
        check(false);
      }, false, {
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  window.rch_nws[hostkey].typeInvoke('http://api.xsena.red', function() {
    ...
    client.on("RchClient", function(rchId, url, data, headers, returnHeaders) {
	  ...
	  function sendResult(uri, html) {
	    $.ajax({
	      url: 'http://api.xsena.red/rch/' + uri + '?id=' + rchId,
	      type: 'POST',
	      data: html,
	      async: true,
	      cache: false,
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript(["http://api.xsena.red/js/nws-client-es5.js?v03022026"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  function View(object) {
    ...
    var Search = {
      ...
      search: function search(params, oncomplite) {
        network.silent('http://api.xsena.red/rch/check/connected', function(json) {
          if (json.rch) {
            rchRun(json, function() {
